    ...
    for conn in elems:
        ...
        for xi in gp:
            for eta in gp:
                ...
                if detJ <= 0:
                    raise ValueError("Invalid Q4 geometry in thermoelastic solver")
                ...
                Tgp = float(N @ Te)
                dT = Tgp - Tref
                eps_th = alpha * dT * np.array([1.0,1.0,0.0], float)
                w = detJ * t
                Ke += B.T @ D @ B * w
                Fe_th += B.T @ (D @ eps_th) * w
        dofs = []
        for n in conn:
