...
def solve_plane_stress2d(env):
    ...
    D = _D_plane_stress(E, nu, t)

    elem_data = []
    for e,conn in enumerate(elems):
        xe = nodes[conn,:]
        if etype.upper()=="T3" and conn.size!=3:
            raise ValueError("T3 needs 3 nodes per element")
        if etype.upper()=="Q4" and conn.size!=4:
    ...
    # traction on edges (uniform tx,ty per unit length)
    for tr in env.get("traction", []):
        ...
        conn = elems[ee]
        if conn.size == 3:   # T3 edges
            edge_nodes = {0:(0,1), 1:(1,2), 2:(2,0)}[edge]
        else:                # Q4 edges
            edge_nodes = {0:(0,1), 1:(1,2), 2:(2,3), 3:(3,0)}[edge]
    ...
    # stresses (nodal average) + element stresses
    sigma_elem = []
    ...
    sigma_vm = np.sqrt(sx*sx + sy*sy - sx*sy + 3.0*txy*txy)

    return {"U": U, "nodes": nodes, "elems": elems, "etype": etype.upper(),
