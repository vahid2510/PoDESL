...
def _q4_shape(xi, eta):
    ...
    return N,dN_dxi
def solve_heat_axi_transient(env):
    ...
    for conn in elems:
        ...
        for xi in gp:
            for eta in gp:
                ...
                r_g=float(N @ xe[:,0]); w=2*np.pi*r_g*detJ
                B=dN_dx
                Ke += k*(B @ B.T)*w
    ...
    for spec in env.get('hedge',[]):
        ...
        conn=elems[ee]; edge_nodes=[(0,1),(1,2),(2,3),(3,0)][edge]; a,b=edge_nodes
        n1,n2=conn[a],conn[b]; r1,z1=nodes[n1]; r2,z2=nodes[n2]
        L=float(np.hypot(r2-r1,z2-z1)); rmid=0.5*(r1+r2); w=2*np.pi*rmid
        Ke=(h*L/6.0)*w*np.array([[2,1],[1,2]],float); Fe=(h*Tinf*L/2.0)*w*np.array([1.0,1.0],float)
        idx=[n1,n2]
    ...
    if fixed_idx.size>0:
        Afree=A[np.ix_(free_idx,free_idx)]; Acf=A[np.ix_(free_idx,fixed_idx)]
    else:
        Afree=A; Acf=None
